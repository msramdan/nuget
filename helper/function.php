...
function login($u, $p)
{
    global $koneksi;
    $p = sha1($p);
    $q = mysqli_query($koneksi, "SELECT a.*, l.level FROM account a JOIN level l ON l.id_level = a.id_level WHERE a.username='$u' AND a.password='$p'");
...
function getAllNumberKategori($idkategori)
{
    global $koneksi;
    $q = mysqli_query($koneksi, "SELECT * FROM `nomor` where `id_kategori`='$idkategori'");
    $arr = [];
...
function url_img()
{
    return getSingleValDB("pengaturan", "id", "1", "imgUrl");
}
...
function sendMSGbtn($number, $msg, $btn1, $btn2)
{
    $url = url_wa() . "v2/send-message-button";
    $data = [
        "number" => $number,
        "message" => $msg,
        "button1" => $btn1,
...
function sendMSGlist($number, $msg)
{
    $url = url_wa() . "v2/send-message-list";
    $data = [
        "number" => $number,
        "message" => $msg
    ];
...
function sendMedia($number, $filetype, $msg, $file)
{
    $url = url_wa() . "v2/send-media";
    $data = [
        'number' => $number,
